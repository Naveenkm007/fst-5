...
const initialRequests = [
  ...
  { id: 3, type: 'medical', dates: '24 Sep - 25 Sep 2026', days: 2, reason: 'Semester health check-up', status: 'Pending' },
]
...
function App() {
  ...
  const reviewRequest = (requestId, status) => {
    ...
    setEvents((current) => [{ id: Date.now(), time: 'Just now', title: `${leaveTypes[request.type].label} ${status.toLowerCase()}`, detail: `${request.days} ${request.days === 1 ? 'day' : 'days'} for Naveen Kumar KM · ${request.dates}.`, tone: status === 'Approved' ? 'green' : 'coral' }, ...current])
  }
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav-list" aria-label="Main navigation">
          ...
          <button className={activeView === 'admin' ? 'nav-item active' : 'nav-item'} onClick={() => setActiveView('admin')}><span>✓</span> Teacher admin <b>{pendingRequests.length}</b></button>
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          ...
          </> : activeView === 'admin' ? <section className="admin-page"><div className="admin-heading"><div><p className="eyebrow">Teacher workspace · NHCE CSE</p><h2>Leave approvals</h2><p className="subtle">Review requests from your class and keep attendance records current.</p></div><div className="teacher-profile"><span className="teacher-avatar">DR</span><span><strong>Dr. R. Meenakshi</strong><small>Class teacher · CSE</small></span></div></div><div className="admin-stats"><div className="admin-stat"><span className="stat-icon coral-bg">⌁</span><div><strong>{pendingRequests.length}</strong><small>Awaiting review</small></div></div><div className="admin-stat"><span className="stat-icon blue-bg">▣</span><div><strong>{requests.length}</strong><small>Total requests</small></div></div><div className="admin-stat"><span className="stat-icon green-bg">✓</span><div><strong>{requests.filter((request) => request.status === 'Approved').length}</strong><small>Approved this term</small></div></div></div><div className="panel approval-panel"><div className="panel-heading"><div><p className="eyebrow">Action queue</p><h2>Pending requests</h2></div><span className="queue-label">{pendingRequests.length} to review</span></div>{pendingRequests.length ? <div className="approval-list">{pendingRequests.map((request) => <div className="approval-row" key={request.id}><span className={`request-badge ${leaveTypes[request.type].color}`}>{leaveTypes[request.type].short}</span><div className="approval-detail"><div><strong>Naveen Kumar KM</strong><span className="class-tag">1NH24CS409 · CSE</span></div><span>{leaveTypes[request.type].label} · {request.dates} · {request.days} {request.days === 1 ? 'day' : 'days'}</span><p>{request.reason}</p></div><div className="approval-actions"><button className="reject-button" onClick={() => reviewRequest(request.id, 'Rejected')}>Reject</button><button className="approve-button" onClick={() => reviewRequest(request.id, 'Approved')}>Approve</button></div></div>)}</div> : <div className="empty-queue"><span>✓</span><strong>All caught up</strong><p>There are no leave requests waiting for your review.</p></div>}</div></section> : <section className="panel requests-page"><div className="panel-heading"><div><p className="eyebrow">Leave history</p><h2>My requests</h2><p className="subtle">Every request you've submitted, in one place.</p></div><button className="primary-button" onClick={() => setIsApplyOpen(true)}><span>＋</span> Apply for leave</button></div><div className="request-table">{requests.map((request) => <div className="request-row" key={request.id}><span className={`request-badge ${leaveTypes[request.type].color}`}>{leaveTypes[request.type].short}</span><div className="request-detail"><strong>{leaveTypes[request.type].label}</strong><span>{request.dates} · {request.days} {request.days === 1 ? 'day' : 'days'} · {request.reason}</span></div><span className={`status ${request.status.toLowerCase()}`}>{request.status}</span></div>)}</div></section>}
        </div>
      </main>
